...
const SplitterNode: React.FC<NodeProps> = ({ data }) => {
  const typedData = data as unknown as SplitterNodeData;
  const numOutputs = typedData.num_splits || 3;
  const structure = typedData.structure || 'json_array';

  return (
    <div className="bg-yellow-100 border-2 border-yellow-300 rounded px-3 py-2 min-w-24 max-w-32">
      <div className="text-xs font-medium text-yellow-800">{typedData.name}</div>
      <div className="text-xs text-yellow-700">Splitter</div>
      <div className="text-xs text-yellow-600 mt-1">Outputs: {numOutputs}</div>
      {typedData.model && <div className="text-xs text-gray-600">AI: {typedData.model}</div>}
