...
const SplitterNode: React.FC<NodeProps> = ({ data }) => {
  const nodeData = data as unknown as SplitterNodeData;
  const numOutputs = nodeData.num_splits || 3;
  const structure = nodeData.structure || 'json_array';

  return (
    <div className="bg-yellow-100 border-2 border-yellow-300 rounded px-3 py-2 min-w-24 max-w-32">
      <div className="text-xs font-medium text-yellow-800">{nodeData.name}</div>
      <div className="text-xs text-yellow-700">Splitter</div>
      <div className="text-xs text-yellow-600 mt-1">Outputs: {numOutputs}</div>
      {nodeData.model && <div className="text-xs text-gray-600">AI: {nodeData.model}</div>}
